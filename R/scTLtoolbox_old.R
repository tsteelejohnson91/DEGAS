...
fromOneHot <- function(labMat){
  return(apply(labMat,1,function(x) which(x==1)))
}
...
writeInputFiles <- function(scExp,scLab,patExp,patLab,tmpDir){
  ...
  write.table(scLab,file=paste0(tmpDir, '/scLab.csv'), row.names=FALSE, sep=',')
...
readOutputFiles <- function(tmpDir){
  ...
  return(new('ccModel',Bias=Biases,Theta=Thetas,Activation=Activations,Depth=depth))
}

makeExec <- function(tmpDir,FFdepth,model_type){
  if (model_type != 'ClassClass' && model_type != 'CoxClass'){
    stop("Please specify either 'ClassClass' or 'CoxClass' for the model_type")
  }
...
runCCMTL <- function(scExp,scLab,patExp,patLab,tmpDir,model_type,architecture,FFdepth){
  ...
  ccModel1 = readOutputFiles(tmpDir)
  #system(paste0('rm -rf ',tmpDir))
  return(ccModel1)
}

predClass <- function(ccModel1,Exp,scORpat,architecture){
  if(architecture=="DenseNet"){
