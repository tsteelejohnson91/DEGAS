...
setClass("ccModel",slots=list(Bias="list",Theta="list",Activation="list",
                              Depth="numeric",model_type="character"))
...
readOutputFiles <- function(tmpDir,model_type){
  ...
  return(new('ccModel',Bias=Biases,Theta=Thetas,Activation=Activations,Depth=depth,model_type=model_type))
}
...
runCCMTL <- function(scExp,scLab,patExp,patLab,tmpDir,model_type,architecture,FFdepth){
  ...
  ccModel1 = readOutputFiles(tmpDir,model_type)
  #system(paste0('rm -rf ',tmpDir))
  return(ccModel1)
}
