...
loss = 2*lossLabel1 + lambda3*lossMMD
...
tensor_train = {xs: np.concatenate([np.squeeze(Xsc[train_sc2,]),np.squeeze(Xpat[train_pat2,:])]), ys_sc: np.squeeze(Ysc[train_sc2,:]), lsc: len(train_sc2), lpat: len(train_pat2), kprob: do_prc}
...
for i in range(train_steps+1):
	...
	if(i % 50 == 0):
		print(str(sess.run(loss, feed_dict=tensor_train))+' '+str(sess.run(lossLabel1,feed_dict=tensor_train))+'	'+str(sess.run(lossMMD,feed_dict=tensor_train)))
		train_sc = resample(50,Ysc,idx_sc)			# CHANGED
		#train_pat = resample(50,Ypat,idx_pat)		# CHANGED
